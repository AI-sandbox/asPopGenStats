...
argp.add_argument(
    "stat",
    help="the statistics to compute (H stands for heterozygosity)",
...
argp.add_argument(
    "-t", "--data_dir", required=True, help="(REQUIRED) frequency data file directory"
)
argp.add_argument(
    "-o", "--output_dir", required=True, help="(REQUIRED) output file directory"
)
argp.add_argument(
    "-b", "--blocksize", type=int, default=500, help="block size for block bootstrap"
)
argp.add_argument(
    "-n",
    "--num_replicates",
...
class StatisticsParameters(object):
    def __init__(self, args_):
        ...
        self.save_dir = os.path.join(args_.output_dir, f"{self.stats_name:s}_output")
        ...
        if (
            ((self.pop_size == 1) and num_pop_files != 1)
            or ((self.pop_size == 2) and (num_pop_files not in [1, 2]))
            or ((self.stats_name == "f3") and (num_pop_files not in [1, 2]))
            or ((self.stats_name == "f4") and num_pop_files != 3)
        ):
            print(
                "Please specify a correct number of population list files for "
...
def aggregate_ref_pop(
    data_dir, stats_name, pop_data_files, aggr_filename, DA_filename, thres
):
    """
    ...
    if (not os.path.exists(aggr_file_abspath)) or (
        ...
        print(
            "Reference population groups detected without "
            "an aggregated population file or for Psi, a derived allele SNP "
            "position file."
        )
        print(
            "Creating the aggregated population file (and for Psi, its "
            "corresponding derived allele SNP position file)..."
        )
        paggr.construct_aggr_file(
            data_dir,
            stats_name,
            pop_data_files,
            aggr_file_abspath,
...
def read_ref_pop_info(params, args_):
    ...
    if stats_name in ["f3", "f4", "psi"]:
        try:
            ...
            if args_.ref_group != None:
                ...
                print(
                    "Please specify the reference (outgroup) populations "
                    "for F3/F4/psi statistics (first-letter capitalized, "
                    "separated by space, if many, e.g., Atayal Bunun Paiwan)"
                )
                print(
                    "(If there is more than one population, we will create "
            ...
            if stats_name == "psi":
                ...
                while thres < -0.0 or thres > 0.5:
                    print(
                        "Please specify derived allele frequency threshold "
            ...
            if len(outgroup_name) == 1:
                ...
                aggr_filename = (
                    "".join(
                        [
                            pop.strip().split(".")[0][:3].capitalize()
            ...
            if stats_name == "psi":
                ...
                DA_filename = (
                    f"psi_{aggr_filename.split('.')[0]:s}_frq_" f"{DAF_str:s}.txt"
                )

            # Aggregate specified reference population into one frequency file.
            pop_data_files = [pop + ".freq" for pop in outgroup_name]
            aggr_file_abspath, DA_file_abspath = aggregate_ref_pop(
                data_dir, stats_name, pop_data_files, aggr_filename, DA_filename, thres
            )
    ...
    params.output_file_abspath = os.path.join(
        params.save_dir, f"{stats_name:s}_stat" f"{output_file_suffix:s}.txt"
    )
...
def stats_input(pops, params):
    ...
    )  # Name for the output text file (.txt) of the statistics
    outgroup_filename = (
        params.aggr_file_abspath
    )  # (F3/Psi) Frequency file name for the outgroup populations
    ...
    if stats_name == "heterozygosity":
        ...
        config = gst.QuaternaryInputConfig(
            data_dir=data_dir,
            pop1_file=pops[0],
            pop2_file=pops[1],
            block_size=block_size,
...
class UnaryComputation(object):
    def __init__(self, params):
        ...
        if num_pop_files != 1:
            print(
                "The unary input genetic statistics requires 1 population " "list file."
            )
            sys.exit(0)
    ...
    def save_output_file(self, params, out_mtx, out_mtx_se, populus_list1):
        stat_mtx_file = (
            f"{params.save_dir:s}/{params.stats_name:s}_mtx"
...
class GeneralComputation(object):
    def __init__(self, params):
        ...
        if num_pop_files not in [1, 2]:
            print(
                "The input genetic statistics requires 1 or 2 population " "list files."
            )
            sys.exit(0)
    ...
    def validate_inputs(self, params):
        ...
        ref = pd.read_csv(
            freq_files[0], usecols=["CHR", "SNP", "A1"], sep="\t", header=0
        )
        for i in range(1, len(freq_files)):
            curr_pop = pd.read_csv(
                freq_files[i], usecols=["CHR", "SNP", "A1"], sep="\t", header=0
            )
            if not ref.equals(curr_pop):
                print(
                    "All population frequency files must contain the exact same SNPs and minor alleles. "
    ...
    def self_match_computation(self, params):
        ...
        for i in trange(num_pop1):
            ...
            for j in range(i + 1, num_pop1):
                ...
                print(f"{pop1:s}-{pop2:s}: " f"{out_mtx[i][j]:f}, {out_mtx_se[i][j]:f}")
        ...
        self.save_output_file(params, out_mtx, out_mtx_se, populus_list1, populus_list1)
    ...
    def cross_match_computation(self, params):
        ...
        for i in trange(num_pop1):
            ...
            for j in range(num_pop2):
                ...
                out_mtx[i][j], out_mtx_se[i][j] = stats_mean, stats_std_error
                print(f"{pop1:s}-{pop2:s}: " f"{out_mtx[i][j]:f}, {out_mtx_se[i][j]:f}")
        self.save_output_file(params, out_mtx, out_mtx_se, populus_list1, populus_list2)

    def save_output_file(
        self, params, out_mtx, out_mtx_se, populus_list1, populus_list2
    ):
        stat_mtx_file = (
            f"{params.save_dir:s}/{params.stats_name:s}_mtx"
            f"{params.output_file_suffix:s}"
...
class F4Computation(object):
    def __init__(self, params):
        ...
        if num_pop_files != 3:
            print(
                "The genetic statistics F4 requires 3 population "
    ...
    def cross_match_computation(self, params):
        ...
        for k in trange(num_pop4):
            ...
            for i in range(num_pop1):
                ...
                for j in range(num_pop2):
                    pop2 = populus_list2[j]
                    stats_mean, stats_std_error = stats_input(
                        [pop1, pop2, pop4], params
                    )
                    out_mtx[i][j][k], out_mtx_se[i][j][k] = stats_mean, stats_std_error
                    print(
                        f"F4({pop1:s}, {pop2:s}; O, {pop4:s}): "
                        f"{out_mtx[i][j][k]:f}, {out_mtx_se[i][j][k]:f}"
                    )
        self.save_output_file(
            params, out_mtx, out_mtx_se, populus_list1, populus_list2, populus_list4
        )

    def save_output_file(
        self, params, out_mtx, out_mtx_se, populus_list1, populus_list2, populus_list4
    ):
        stat_mtx_file = (
            f"{params.save_dir:s}/{params.stats_name:s}_mtx"
            f"{params.output_file_suffix:s}"
...
if __name__ == "__main__":
    ...
    subprocess.check_output([f"touch {stat_params.output_file_abspath:s}"], shell=True)
