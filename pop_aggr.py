...
def construct_aggr_file(
    data_dir, stats_name, files, aggr_file_abspath, DA_file_abspath, DAF
):
    """
    ...
    """
    # ["CHR", "SNP", "A1", "A2", "MAF", "NCHROBS", "MAF_MSK", "NCHROBS_MSK"]
    colname = ["MAF", "NCHROBS", "MAF_MSK", "NCHROBS_MSK"]
    colname_chrom = ["CHR", "SNP", "A1", "A2"]
    # Load and stack the population files.
    curr_pop = pd.read_csv(os.path.join(data_dir, files[0]), header=0, sep="\t")
    chrom_idx = curr_pop[colname_chrom]
    # data frame with column name ["MAF", "NCHROBS", "MAF_MSK", "NCHROBS_MSK"]
    curr_pop_stat = curr_pop[colname].to_numpy()
    res = np.stack(
        (
            curr_pop_stat[:, 0] * curr_pop_stat[:, 1],
            curr_pop_stat[:, 1],
    ...
    if len(files) > 1:
        for i in range(1, len(files)):
            curr_pop_stat = pd.read_csv(
                os.path.join(data_dir, files[i]), header=0, sep="\t"
            )[colname].to_numpy()
            res += np.stack(
                (
                    curr_pop_stat[:, 0] * curr_pop_stat[:, 1],
        ...
        res[:, 0] = np.divide(
            res[:, 0], res[:, 1], out=np.zeros(res.shape[0]), where=(res[:, 1] != 0)
        )  # ["MAF", "NCHROBS"]
        res[:, 2] = np.divide(
            res[:, 2], res[:, 3], out=np.zeros(res.shape[0]), where=(res[:, 3] != 0)
        )  # ["MAF_MSK", "NCHROBS_MSK"]
        res = res.astype("float32")

        df_final = pd.DataFrame(
            np.hstack((chrom_idx, res)),
            columns=[
                "CHR",
                "SNP",
                "A1",
...
def construct_DA_file(df, data_dir, DA_file_abspath, DAF):
    ...
    if abs(DAF - 0.5) <= 1e-5:
        ...
        stream_output.append(
            f"Number of DAF {DAF} (TYPE 1): " f"{np.sum(df[:, 0] <= DAF)}"
        )
        stream_output.append(
            f"Number of DAF {1-DAF} (TYPE 2): " f"{np.sum(df[:, 0] >= (1-DAF))}"
        )
