...
labels = data["labels"]
df = pd.DataFrame(
    data["masks"][0][ancestry], index=list(rs_ID_list), columns=list(ind_ID_list)
)
df = df.sort_index()
print(df.head(5))
...
def create_freq(nation, nation_dict, df, rs_ID_list, save_file_path):
    ...
    df_freq = np.divide(df_sum, df_count, np.zeros(df_sum.shape), where=(df_count != 0))
    np_data = np.vstack((rs_ID_list, df_freq, df_count)).T
    df_final = pd.DataFrame(np_data, columns=["CHROM_IDX", "FREQ", "CT"])
    df_final.to_csv(os.path.join(save_file_path, f"{nation:s}.freq"), index=False)


# Create frequency files for all nations.
remote_elements = [
    create_freq.remote(nation, nation_dict, df, rs_ID_list, save_file_path)
    for nation in nation_dict.keys()
]
